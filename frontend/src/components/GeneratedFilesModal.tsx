...
export const GeneratedFilesModal: React.FC<GeneratedFilesModalProps> = ({ inline = false }) => {
  ...
  const content = (
    <div className={`w-full ${inline ? 'h-full flex flex-col' : 'max-w-3xl max-h-[85vh]'} bg-[#1a1614] border-2 border-[#3a312c] rounded-2xl p-6 shadow-2xl space-y-4`}>
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#3a312c] pb-3 shrink-0">
        <div>
      ...
      <div className="flex items-center gap-2 border-b border-[#3a312c] pb-2 overflow-x-auto shrink-0">
        {files.map((file, idx) => (
          <button
            key={file.filename}
            onClick={() => setActiveFileIndex(idx)}
            className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg font-mono text-[12px] font-bold border transition ${
              activeFileIndex === idx
      ...
      <div className="relative flex-1 min-h-0 flex flex-col group">
        ...
        <pre className="flex-1 w-full bg-[#120f0e] border border-[#3a312c] p-4 pt-12 rounded-xl font-code text-[12px] text-amber-300 overflow-auto leading-relaxed">
          {activeFile.content}
        </pre>
      </div>
